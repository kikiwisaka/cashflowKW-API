...
{
    public class AssetDataController : BaseAPIController
    {
        private readonly IAssetDataService _assetDataService;

        public AssetDataController(IAssetDataService assetDataService)
        {
        ...
        //GET api/assetData
        [HttpGet]
        public IHttpActionResult Get([FromUri] AssetDataListParameter param)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    int totalRows = _assetDataService.GetAll().Count();
                    var totalPage = totalRows / param.PageSize;
                    var totalPages = (int)Math.Ceiling((double)totalRows / param.PageSize);
                    if (totalPages < 0)
                        totalPages = 0;

                    var result = _assetDataService.GetAll()
                        .Skip(skip)
                        .Take(param.PageSize)
                        .ToList();

                    IList<AssetDataDTO> colls = AssetDataDTO.From(result);
        ...
        public IHttpActionResult Get(int id)
        {
            try
            {
                var result = _assetDataService.Get(id);
                AssetDataDTO assetDataDTO = AssetDataDTO.From(result);
        ...
        //POST api/assetData
        [HttpPost]
        public IHttpActionResult Add(AssetDataParam param)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    int id = _assetDataService.Add(param);
                    return Ok(id);
                }
                else
                {
        ...
        //PUT api/assetData/id
        [HttpPut]
        public IHttpActionResult Update(int id, [FromBody]AssetDataParam param)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    int result = _assetDataService.Update(id, param);
                    return Ok(result);
                }
                else
                {
        ...
        public IHttpActionResult Delete(int id)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    int userId = UserHelper.GetCurrentUserId();
                    DateTime getDate = DateHelper.GetDateTime();
                    int result = _assetDataService.Delete(id, userId, getDate);
                    return Ok(result);
                }
                else
                {
